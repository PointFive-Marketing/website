(function() {
    ...
    function injectHTML() {
        ...
        if (!container) {
            console.error('❌ Container element not found. Make sure you have: <div id="conversion-landing-container"></div>');
            return;
        }

        if (document.getElementById('conversion-landing-wrapper')) {
            console.log('⚠️ Content already loaded');
            return;
        }
    ...
    // Initialize
    function init() {
        console.log('🚀 PointFive Conversion Loader initializing...');
        
        loadTailwind();
        injectCSS();
        
        // Wait for Tailwind to be available (simpler check)
        let attempts = 0;
        const maxAttempts = 50; // 5 seconds max wait
        
        const checkAndInject = setInterval(function() {
            attempts++;
            
            // Check if Tailwind script tag exists and some time has passed
            const tailwindScript = document.querySelector('script[src*="tailwindcss"]');
            
            if (tailwindScript || attempts > 10) {
                clearInterval(checkAndInject);
                // Small delay to ensure Tailwind is ready
                setTimeout(function() {
                    injectHTML();
                }, 200);
            }
            ...
            if (attempts >= maxAttempts) {
                clearInterval(checkAndInject);
                console.error('❌ Timeout waiting for Tailwind. Injecting HTML anyway...');
                injectHTML();
            }
        }, 100);
    }
